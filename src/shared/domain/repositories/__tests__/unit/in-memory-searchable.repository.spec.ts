...
class StubEntity extends EntityGeneric<StubEntityProps> {}
...
describe('InMemoryRepository unit tests', () => {
  ...
  describe('applyPaginate method', () => {
    it('should paginate items', async () => {
      ...
      let itemsPaginated = await sut['applyPaginate'](items, 1, 2) //Retorna 2 página, 2 items
      expect(itemsPaginated).toStrictEqual([items[0], items[1]])

      itemsPaginated = await sut['applyPaginate'](items, 2, 2) //Retorna 2 página, 2 items
      expect(itemsPaginated).toStrictEqual([items[2], items[3]])

      itemsPaginated = await sut['applyPaginate'](items, 3, 2) //Retorna 3 página, 2 items
      expect(itemsPaginated).toStrictEqual([items[4]])

      itemsPaginated = await sut['applyPaginate'](items, 4, 2) //Retorna 4 página, 2 items
